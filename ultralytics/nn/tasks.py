    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3):
                ...
                n = 1
        # -------------Mine-----------------
        elif m in (PyramidUpfusion, PyramidDownfusion, MultGLPAN, MultGLFPN,
                   PyramidUpfusionv2, PyramidDownfusionv2, MultGLFPNc, MultGLPANc,
                   PyramidfusionP4):
            c1 = [ch[x] for x in f]
...
def yaml_model_load(path):
    ...
    unified_path = re.sub(r'(SPAFPN)([nslmx])(.+)?$', r'\1\3', unified_path)  # for SPAFPN-{   }.yaml
...
def guess_model_scale(model_path):
    ...
    with contextlib.suppress(AttributeError):
        import re
        if str(model_path).startswith('SPAFPN'):
            return re.search(r'SPAFPN+([nslmx])', Path(model_path).stem).group(1)  # n, s, m, l, or x
        else:
