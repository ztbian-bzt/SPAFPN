...
def select_device(device='', batch=0, newline=False, verbose=True):
    ...
    s = f'Scarf PAFPN: Ultralytics v{__version__} Python-{platform.python_version()} torch-{torch.__version__} '
    ...
    if cpu or mps:
        ...
        if not (torch.cuda.is_available() and torch.cuda.device_count() >= len(device.replace(',', ''))):
            LOGGER.info(s)
            LOGGER.info(f'*****  Thanks for open source code of Ultralytics  *****')
